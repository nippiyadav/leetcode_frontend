...
function ProblemPage() {
  ...
  return (
    <div className='p-4 grid grid-cols-5 gap-5 h-[94vh] bg-gray-800/90'>
        <div className='basis-[200px] bg-gray-900 row-span-full'></div>
        <div className='flex-1 col-span-3'>
            {leetCodeProblems?.length > 0 ? leetCodeProblems?.map((v,i)=>(
              <Link key={i} to={`/execution/${v.id}`}>
                <div key={i} className='commanFlex justify-between mb-2 text-white bg-gray-900 p-2 rounded-md flex-wrap even:bg-gray-800'>
